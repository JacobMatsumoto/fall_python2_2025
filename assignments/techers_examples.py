...
class MouseClickDemo:
    ...
    def handle_left_click(self, event):
        ...
        self.click_label.config(
            text=f"Left clicked at: x={event.x}, y={event.y}")

    def handle_right_click(self, event):
        """Called when right mouse button is clicked"""
        self.click_label.config(
            text=f"Right clicked at: x={event.x}, y={event.y}")
...
class KeyPressDemo:
    ...
    def __init__(self, parent):
        ...
        self.key_frame = tk.Frame(
            frame, bg="lightyellow", width=400, height=60)
        ...
        self.key_frame.bind(
            '<FocusIn>', lambda e: self.key_frame.config(bg="lightyellow"))
        self.key_frame.bind(
            '<FocusOut>', lambda e: self.key_frame.config(bg="lightgray"))
    ...
    def handle_key_press(self, event):
        ...
        self.key_label.config(
            text=f"Key: '{event.char}' | Symbol: {event.keysym} | Code: {event.keycode}")
    ...
    def toggle_f1_binding(self):
        ...
        if self.f1_bound:
            ...
            self.toggle_button.config(
                text="Disable F1 (unbind)", bg="lightgreen")
...
class AfterDemo:
    ...
    def show_delayed_message(self):
        ...
        self.root.after(3000, lambda: messagebox.showinfo(
            "Delayed", "3 seconds have passed!"))

        # Reset the label after 4 seconds
        self.root.after(4000, lambda: self.after_label.config(
            text=f"Counter: {self.after_counter}"))
    ...
    def update_countdown(self):
        ...
        if self.countdown_value > 0:
            self.countdown_label.config(
                text=f"Countdown: {self.countdown_value}")
...
class DestroyDemo:
    ...
    def create_dynamic_button(self):
        ...
        new_button.config(
            command=lambda btn=new_button: self.destroy_button(btn))
...
class FocusDemo:
    ...
    def __init__(self, parent):
        ...
        tk.Label(entry_frame, text="Username:", bg="white").grid(
            row=0, column=0, padx=5, pady=5)
        self.entry1 = tk.Entry(entry_frame, width=20)
        self.entry1.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(entry_frame, text="Password:", bg="white").grid(
            row=1, column=0, padx=5, pady=5)
        self.entry2 = tk.Entry(entry_frame, width=20, show="*")
        self.entry2.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(entry_frame, text="Email:", bg="white").grid(
    ...
    def submit_form(self):
        ...
        messagebox.showinfo(
            "Submit", "Form would be submitted!\n(Pressing Enter in Email field)")
...
class ObservableVariablesDemo:
    ...
    def __init__(self, parent):
        ...
        tk.Label(frame, text="Type here:", bg="white").grid(
            row=1, column=0, padx=5, pady=5)
        ...
        tk.Label(frame, text="Move slider:", bg="white").grid(
    ...
    def on_string_var_change(self, *args):
        ...
        self.string_display.config(
            text=f"Value: '{new_value}' (length: {length})")
    ...
    def on_int_var_change(self, *args):
        ...
        self.int_display.config(
            text=f"Value: {new_value} | Doubled: {doubled}")
